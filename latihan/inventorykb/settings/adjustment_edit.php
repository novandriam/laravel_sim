...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtCat'])=="") {
			$message[] = "<b>Catatan penyesuaian barang belum diisi</b>, silahkan diisi!";		
		}
		...
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE tmp_adjusment SET kode='$txtKode', jumlah='$txtJumlah', cat='$txtCat' WHERE id='".$_POST['txtID']."'") 
					   or die ("Gagal query update".mysql_error());
			if($qryUpdate){
				echo "<meta http-equiv='refresh' content='0; url=?page=Add-Penyesuaian'>";
			}
			exit;
		}	
	...
	$sqlShow = "SELECT barang.kd_barang, barang.nm_barang, barang.satuan, tmp_adjusment.* 
	FROM tmp_adjusment inner join barang on tmp_adjusment.kode=barang.kd_barang
	WHERE tmp_adjusment.id='$IDEdit'";
	...
	$dataCat	= isset($dataShow['cat']) ?  $dataShow['cat'] : $_POST['txtCat'];

} // Penutup GET
?>
<form action="?page=Edit-Penyesuaian" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr><input name="txtID" type="hidden" value="<?php echo $dataID; ?>" />
	  <td colspan="3"><h2>UBAH  DETAIL DATA PENYESUAIAN BARANG</h2></td>
	</tr>
    ...
    <tr>
	  <td><b>Catatan</b></td>
	  <td><b>:</b></td>
	  <td><input name="txtCat" type="text" value="<?php echo $dataCat; ?>" size="80" maxlength="100" onKeyPress="return tabE(this,event)" /></td>
    </tr>
