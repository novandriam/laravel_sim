...
<fieldset style="width:90%"><legend>Closing Barang Modal</legend>
<p>
...
<?php
   if($_POST[submit]=="Closing"){
           ...
           $cek = mysql_query("select T1.kd_barang, T1.kd_kategori, T2.kd_barang, T2.tgl_closing, T2.id as jml from barang as T1,closing as T2 where T2.kd_barang=T1.kd_barang and T1.kd_kategori='K03' and T2.tgl_closing='".InggrisTgl($_POST['cmbTglclosing'])."' limit 1");
           ...
           if($rcek['jml']>0){
			   ...
			   // untuk closing bahan baku
                   //cari item barang
                   $brg = mysql_query("select kd_barang,nm_barang,kd_kategori from barang where kd_kategori='K03'");
