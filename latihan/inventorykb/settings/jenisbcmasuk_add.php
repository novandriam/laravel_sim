...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		$sqlCek="SELECT * FROM jenisbcmasuk WHERE jenis='$txtjenis'";
		...
		if(count($message)==0){			
			$NewID	= buatKode("jenisbcmasuk", "BCIN");
			$qrySave=mysql_query("INSERT INTO jenisbcmasuk SET kode='$NewID', jenis='$txtJenis', nama='$txtNama'") 
					 or die ("Gagal query".mysql_error());
			if($qrySave){
				$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menambah data jenis dokpab masuk dengan jenis $txtJenis dan nama $txtNama', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
				echo "<meta http-equiv='refresh' content='0; url=?page=Data-Jenisbcmasuk'>";
			}
			exit;
		}	
	...
	$dataKode	= buatKode("jenisbcmasuk", "BCIN");
	$dataJenis	= isset($_POST['txtJenis']) ? $_POST['txtJenis'] : '';
	$dataNama	= isset($_POST['txtNama']) ? $_POST['txtNama'] : '';

} // Penutup GET
?>
<form action="?page=Add-Jenisbcmasuk" method="post" name="frmadd" target="_self">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>TAMBAH JENIS DOKPAB MASUK</h2> </td>
	</tr>
	<tr>
