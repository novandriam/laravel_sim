...
if($_GET) {
	...
	if (trim($_POST['txtNama'])=="") {
		...
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE jenisbcmasuk SET jenis='$txtJenis', nama='$txtNama'	WHERE kode ='".$_POST['txtKode']."'") 
					   or die ("Gagal query update".mysql_error());
			if($qryUpdate){
					$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Mengubah data jenis dokpab masuk dengan jenis $txtJenis dan nama $txtNama', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
	...
	$sqlShow = "SELECT * FROM jenisbcmasuk WHERE kode='$KodeEdit'";
	$qryShow = mysql_query($sqlShow, $koneksidb)  or die ("Query ambil data jenis dokpab masuk salah : ".mysql_error());
...
?>
<form action="?page=Edit-Jenisbcmasuk" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>UBAH JENIS DOKPAB MASUK</h2> </td>
	</tr>
	<tr>
