...
<fieldset style="width:90%"><legend>Closing Stock</legend>
<p>
...
<input onClick="return confirm('ANDA YAKIN AKAN MENYIMPAN DATA CLOSING, DATA YANG TERSIMPAN TIDAK DAPAT DIUBAH/DIHAPUS?')" type="submit" name="submit" value="Closing">
...
<?php
   if($_POST[submit]=="Closing"){
           ...
           $cek = mysql_query("select count(*) as jml from closing where tgl_closing='".InggrisTgl($_POST['cmbTglclosing'])."' limit 1");
           $rcek = mysql_fetch_array($cek);
           if($rcek[jml]>0){
                   echo "<script>alert(\"Tanggal Tersebut sudah pernah melakukan closing!!!\");</script>";
           }else{
			   
			   // untuk closing bahan baku
                   //cari item barang
                   $brg = mysql_query("select kd_barang,nm_barang,kd_kategori from barang");
                   while($rbrg = mysql_fetch_array($brg)){
                        ...
                        if($rclosing[tgl_closing]==""){
							  ...
							  $pemasukanbahanbaku = @mysql_query("select sum(jumlah) as jml from incoming_terima_detail where kode='$rbrg[kd_barang]' AND dari='PRODUKSI' and tgl_lpb < '".InggrisTgl($_POST['cmbTglclosing'])."'");
                              $rpemasukanbahanbaku = @mysql_fetch_array($pemasukanbahanbaku);
                        ...
                        $stock = $rclosing[stock]+$rpemasukan[jml]-$rpengeluaran[jml]+$radjust[jml];
                        $sql = @mysql_query("insert into closing values('".InggrisTgl($_POST['cmbTglclosing'])."','$rbrg[kd_barang]',$stock)");
