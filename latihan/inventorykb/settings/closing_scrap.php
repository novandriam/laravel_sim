...
<fieldset style="width:90%"><legend>Closing Bahan Sisa / Scrap</legend>
<p>
...
<?php
   if($_POST[submit]=="Closing"){
           ...
           $cek = mysql_query("select T1.kd_barang, T1.kd_kategori, T2.kd_barang, T2.tgl_closing, T2.id as jml from barang as T1,closing as T2 where T2.kd_barang=T1.kd_barang and T1.kd_kategori='K04' and T2.tgl_closing='".InggrisTgl($_POST['cmbTglclosing'])."' limit 1");
           ...
           if($rcek['jml']>0){
			   ...
			   // untuk closing bahan baku
                   //cari item barang
                   $brg = mysql_query("select kd_barang,nm_barang,kd_kategori from barang where kd_kategori='K04'");
                   while($rbrg = mysql_fetch_array($brg)){
                        ...
                        if($rclosing['tgl_closing']==""){
							  ...
							  //cari terima oleh outgoing dari produksi untuk barang hasil produksi			  
							  $hasilproduksi = mysql_query("select sum(jumlah) as jml from outgoing_terima_detail where kode='$rbrg[kd_barang]' AND dari='PRODUKSI' and tgl_lpb < '".InggrisTgl($_POST['cmbTglclosing'])."'");
                              $rhasilproduksi = mysql_fetch_array($hasilproduksi);
							  
							  $total_pemasukan = $rpemasukan['jml']+$rhasilproduksi['jml'];
							  ...
							  //cari terima oleh produksi dari outgoing karena return		  
			   $outgoingreturn = mysql_query("select sum(jumlah) as jml from produksi_terima_detail where kode='$rbrg[kd_barang]' AND dari='OUTGOING' and tgl_lpb < '".InggrisTgl($_POST['cmbTglclosing'])."'");
                              $routgoingreturn = mysql_fetch_array($outgoingreturn);

							 $total_pengeluaran = $rpengeluaran['jml']+$routgoingreturn['jml'];
					...
					//cari terima oleh outgoing dari produksi untuk barang hasil produksi			  
							  $hasilproduksi = mysql_query("select sum(jumlah) as jml from outgoing_terima_detail where kode='$rbrg[kd_barang]' AND dari='PRODUKSI' and tgl_lpb between $rclosing[tgl_closing] and '".InggrisTgl($_POST['cmbTglclosing'])."'");
                              $rhasilproduksi = mysql_fetch_array($hasilproduksi);
			$total_pemasukan = $rpemasukan['jml']+$rhasilproduksi['jml'];
						  ...
						  //cari terima oleh produksi dari outgoing karena return		  
			   $outgoingreturn = mysql_query("select sum(jumlah) as jml from produksi_terima_detail where kode='$rbrg[kd_barang]' AND dari='OUTGOING' and tgl_lpb between $rclosing[tgl_closing] and '".InggrisTgl($_POST['cmbTglclosing'])."'");
                              $routgoingreturn = mysql_fetch_array($outgoingreturn);
 $total_pengeluaran = $rpengeluaran['jml']+$routgoingreturn['jml'];
