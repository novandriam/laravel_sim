...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtJenis'])=="") {
			$message[] = "Jenis dok pabean tidak boleh kosong !";		
		}
	if (trim($_POST['txtNama'])=="") {
			$message[] = "Nama dok pabean tidak boleh kosong !";		
		}
		...
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE jenisbckeluar SET jenis='$txtJenis', nama='$txtNama'	WHERE kode ='".$_POST['txtKode']."'") 
					   or die ("Gagal query update".mysql_error());
			if($qryUpdate){
				$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Mengubah data jenis dokpab keluar dengan jenis $txtJenis dan nama $txtNama', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
	...
	$sqlShow = "SELECT * FROM jenisbckeluar WHERE kode='$KodeEdit'";
	$qryShow = mysql_query($sqlShow, $koneksidb)  or die ("Query ambil data jenis dokpab keluar salah : ".mysql_error());
...
?>
<form action="?page=Edit-Jenisbckeluar" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>UBAH JENIS DOKPAB KELUAR </h2></td>
	</tr>
	<tr>
...
<tr>
	...
	<tr><td>&nbsp;</td>
	  <td>&nbsp;</td>
	  <td><input onclick="return confirm('ANDA YAKIN AKAN MENYIMPAN DATA TRANSAKSI?')" type="submit" name="btnSave" value=" SIMPAN " style="cursor:pointer;">&nbsp;&nbsp;&nbsp;<input type="button" value="Batal" onclick="self.history.back()" /></td>
    </tr>
</table>
</form>
