...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtNpwp'])=="") {
			$message[] = "<b>ID/NPWP Pemasok</b> tidak boleh kosong !";		
		}
		...
		$txtSupplierlama= $_POST['txtSupplierlama'];
		...
		# Validasi npwp pemasok, jika sudah ada akan ditolak
		$sqlNpwp="SELECT * FROM pemasok WHERE npwp='$txtNpwp' AND NOT(npwp='$txtNpwplama')";
		$qryNpwp=mysql_query($sqlNpwp, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryNpwp)>=1){
			$message[] = "Maaf, NPWP/ID pemasok dengan <b> $txtNpwp </b> sudah ada, ganti dengan yang lain";
		}

	# Validasi nama pemasok, jika sudah ada akan ditolak
		$sqlNama="SELECT * FROM pemasok WHERE nama='$txtSupplier' AND NOT(nama='$txtSupplierlama')";
		$qryNama=mysql_query($sqlNama, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryNama)>=1){
			$message[] = "Maaf, Nama pemasok dengan  <b> $txtSupplier </b> sudah ada, ganti dengan yang lain";
		}
		
		# Jika jumlah error message tidak ada, simpan datanya
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE pemasok SET npwp='$txtNpwp', nama='$txtSupplier', alamat='$txtAlamat',kota='$txtKota',negara='$txtNegara',notelp='$txtTelp',nofax='$txtFax',email='$txtEmail', status='$cmbStatus' WHERE kode_pemasok ='".$_POST['txtKode']."'") 
					   or die ("Gagal query update".mysql_error());
			if($qryUpdate){
				$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Mengubah data pemasok/supplier dengan kode: $KodeEdit dan nama $txtSupplier', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
	...
	$dataNama	= isset($dataShow['nama']) ?  $dataShow['nama'] : $_POST['txtSupplier'];
...
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>UBAH DATA PEMASOK</h2> </td>
	</tr>
	<tr>
	...
	<tr>
	  <td><b>NPWP/ID </b></td>
	  <td><b>:</b></td>
	  <td><input name="txtNpwp" type="text" value="<?php echo $dataNpwp; ?>" size="80" maxlength="100" onKeyPress="return tabE(this,event)" />
    <input name="txtNpwplama" type="hidden" value="<?php echo $dataNpwplama; ?>" /></td></tr>

	<tr>
	  <td><b>Nama Pemasok </b></td>
	  <td><b>:</b></td>
	  <td><input name="txtSupplier" type="text" value="<?php echo $dataNama; ?>" size="80" maxlength="100" onKeyPress="return tabE(this,event)" />
    <input name="txtSupplierlama" type="hidden" value="<?php echo $dataNamalama; ?>" /></td></tr>
	<tr>
