...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtJenis'])=="") {
			$message[] = "<b>Jenis kemasan</b> tidak boleh kosong !";		
		}
		if (trim($_POST['txtNama'])=="") {
			$message[] = "<b>Nama kemasan</b> tidak boleh kosong !";		
		}
		...
		$sqlCek="SELECT kd_kemasan FROM kemasan WHERE kd_kemasan='$txtJenis'";
		$qryCek=mysql_query($sqlCek, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryCek)>=1){
			$message[] = "Maaf, Jenis kemasan <b> $txtJenis </b> sudah ada, ganti dengan yang lain";
		}

		# SIMPAN DATA KE DATABASE. Jika jumlah error message tidak ada, simpan datanya
		if(count($message)==0){			
			$NewID	= buatKode("kemasan", "");
			$qrySave=mysql_query("INSERT INTO kemasan SET id_kemasan='$NewID', kd_kemasan='$txtJenis', uraian_kemasan='$txtNama'") 
					 or die ("Gagal query".mysql_error());
			if($qrySave){
				$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menambah data kemasan barang dengan jenis $txtJenis dan nama $txtNama', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
	...
	$dataKode	= buatKode("kemasan", "");
	$dataJenis	= isset($_POST['txtJenis']) ? $_POST['txtJenis'] : '';
	$dataNama	= isset($_POST['txtNama']) ? $_POST['txtNama'] : '';

} // Penutup GET
?>
<form action="?page=Add-Kemasan" method="post" name="frmadd" target="_self">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>TAMBAH KEMASAN BARANG</h2></td>
	</tr>
	<tr>
