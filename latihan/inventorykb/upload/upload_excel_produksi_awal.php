...
for ($i=2; $i<=$baris; $i++)
{
  ...
  $query = "INSERT INTO tmp_stock_produksi VALUES ('$id','$kode', '$jumlah')";
  ...
  if ($hasil) $sukses++;
				...
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Upload-Excel-Produksi-Awal'>";
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_stock_produksi WHERE id='".$_GET['ID']."'", $koneksidb) 
	...
	$message = array();
	$cekSql ="SELECT COUNT(kode) As qty FROM tmp_stock_produksi";
		...
		if ($cekRow['qty'] < 1) {
			...
			# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_stock_produksi ";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel produksi awal
	$itemSql = "INSERT INTO stock_produksi SET kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]'";
					mysql_query($itemSql, $koneksidb) or die ("Gagal Query data produksi: ".mysql_error());
						}
						
						# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("TRUNCATE TABLE tmp_stock_produksi ", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Produksi-Stock'>";
						}
		}
...
<form action="?page=Upload-Excel-Produksi-Awal" method="post"  name="frmadd" enctype="multipart/form-data" target="_self">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>UPLOAD DATA BARANG DARI EXCEL UNTUK STOCK PRODUKSI AWAL</h2></td>
	</tr>
</table>
 ...
 <p>Silakan Pilih File Excel :    <input name="userfile" type="file">
  ...
  <p>
(cari file excel: <span class="style1">template_stock_produksi.xls</span> di folder template excel)    
  </p>
...
$tmpSql ="SELECT T1.*, T2.*, T3.kd_barang,T3.nm_barang,T3.satuan,T3.kd_kategori
		FROM tmp_stock_produksi as T1, kategori as T2, barang as T3
		WHERE T1.kode=T3.kd_barang and T3.kd_kategori=T2.kd_kategori
		ORDER BY T1.kode ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$jml	 = mysql_num_rows($tmpQry);
$dataSql ="SELECT kode FROM tmp_stock_produksi ";
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Upload-Excel-Produksi-Awal&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
<tr bgcolor="#FFFFCC">
