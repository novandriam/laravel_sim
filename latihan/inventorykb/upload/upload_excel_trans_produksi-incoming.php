...
$id = $data->val($i, 1);
  ...
  $query = "INSERT INTO tmp_detail_produksi_incoming VALUES ('$id','$kode', '$jumlah', '$ket')";
  ...
  if ($hasil) $sukses++;
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_detail_produksi_incoming WHERE id='".$_GET['ID']."'", $koneksidb) 
	...
	$message = array();
	$cekSql ="SELECT COUNT(kode) As qty FROM tmp_detail_produksi_incoming";
		...
		if ($cekRow['qty'] < 1) {
			...
			# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_detail_produksi_incoming ";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel tmp produksi-incoming detail
					$itemSql = "INSERT INTO tmp_produksi_incoming SET kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]', userid='".$_SESSION['SES_LOGIN']."'";
					mysql_query($itemSql, $koneksidb) or die ("Gagal Query detail barang : ".mysql_error());
						}
						
						# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("TRUNCATE TABLE tmp_detail_produksi_incoming ", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Produksi-Incoming-Add'>";
						}
		}
...
<form action="?page=Upload-Detail-Produksi-Incoming" method="post"  name="frmadd" enctype="multipart/form-data" target="_self">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>UPLOAD DETAIL DATA BARANG DARI EXCEL UNTUK TRANSAKSI PRODUKSI-INCOMING</h2></td>
	</tr>
     <tr>
    <td colspan="3"  align="right"><a href="template/detailbrg_produksi-incoming.php" target="_self" alt="Buat Template Excel" title="Buat template dalam format excel" ><img src="images/excel.jpg" width="15" height="15"  border="0"  alt="Buat Template Excel" />Buat Template</a></td>
    </tr>
</table>
 ...
 <p>Silakan Pilih File Excel :    <input name="userfile" type="file">
   ...
   <p>
(<span class="style2">klik <strong><a href="template/detailbrg_produksi-incoming.php" target="_self">Buat Template</a></strong> untuk membuat file template dari excel</span>)  </p>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_detail_produksi_incoming.*
		FROM barang, tmp_detail_produksi_incoming
		WHERE tmp_detail_produksi_incoming.kode=barang.kd_barang
		ORDER BY tmp_detail_produksi_incoming.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$jml	 = mysql_num_rows($tmpQry);
$dataSql ="SELECT * FROM tmp_detail_produksi_incoming ";
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Trans-Produksi-Incoming&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Upload-Detail-Produksi-Incoming&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
  <tr bgcolor="#FFFFCC">
