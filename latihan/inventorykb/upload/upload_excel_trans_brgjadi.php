...
$id = $data->val($i, 1);
  ...
  $query = "INSERT INTO tmp_detail_brgjadi
   VALUES ('$id','$kode', '$jumlah', '$ket')";
  ...
  if ($hasil) $sukses++;
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_detail_brgjadi WHERE id='".$_GET['ID']."'", $koneksidb) 
	...
	$message = array();
	$cekSql ="SELECT COUNT(kode) As qty FROM tmp_detail_brgjadi";
		...
		if ($cekRow['qty'] < 1) {
			...
			# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_detail_brgjadi ";
				...
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					...
					$itemSql = "INSERT INTO tmp_brgjadi SET kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', userid='".$_SESSION['SES_LOGIN']."'";
					mysql_query($itemSql, $koneksidb) or die ("Gagal Query detail barang : ".mysql_error());
						}
						
						# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("TRUNCATE TABLE tmp_detail_brgjadi ", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Brgjadi-Add'>";
						}
		}
...
<form action="?page=Upload-Detail-Brgjadi" method="post"  name="frmadd" enctype="multipart/form-data" target="_self">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>UPLOAD DETAIL DATA BARANG JADI DARI EXCEL </h2></td>
	</tr>
     <tr>
    <td colspan="3"  align="right"><a href="template/detailbrg_brgjadi.php" target="_self" alt="Buat Template Excel" title="Buat template dalam format excel" ><img src="images/excel.jpg" width="15" height="15"  border="0"  alt="Buat Template Excel" />Buat Template</a></td>
    </tr>
</table>
 ...
 <p>Silakan Pilih File Excel :    <input name="userfile" type="file">
  ...
  <p>
(cari file excel: <span class="style1">template_detailbrg_brgjadi.xls</span> di folder template excel)    
  </p>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_detail_brgjadi.*
		FROM barang, tmp_detail_brgjadi
		WHERE tmp_detail_brgjadi.kode=barang.kd_barang
		ORDER BY tmp_detail_brgjadi.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$jml	 = mysql_num_rows($tmpQry);
$dataSql ="SELECT * FROM tmp_detail_brgjadi ";
...
?>
  <tr>
    ...
    <td align="center"><?php echo format_angka($tmpRow['jumlah']); ?></div></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Upload-Detail-Brgjadi&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
  <tr bgcolor="#FFFFCC">
