...
$id = $data->val($i, 1);
  ...
  $query = "INSERT INTO tmp_detail_incoming_outgoing VALUES ('$id','$kode', '$jumlah', '$ket')";
  ...
  if ($hasil) $sukses++;
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_detail_incoming_outgoing WHERE id='".$_GET['ID']."'", $koneksidb) 
	...
	$message = array();
	$cekSql ="SELECT COUNT(kode) As qty FROM tmp_detail_incoming_outgoing";
		...
		if ($cekRow['qty'] < 1) {
			...
			# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_detail_incoming_outgoing ";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel tmp incoming-outgoing detail
					$itemSql = "INSERT INTO tmp_incoming_outgoing SET kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]', userid='".$_SESSION['SES_LOGIN']."'";
					mysql_query($itemSql, $koneksidb) or die ("Gagal Query detail barang : ".mysql_error());
						}
						
						# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("TRUNCATE TABLE tmp_detail_incoming_outgoing ", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Incoming-Outgoing-Add'>";
						}
		}
...
<form action="?page=Upload-Detail-Incoming-Outgoing" method="post"  name="frmadd" enctype="multipart/form-data" target="_self">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>UPLOAD DETAIL DATA BARANG DARI EXCEL UNTUK TRANSAKSI INCOMING-OUTGOING</h2></td>
	</tr>
     <tr>
    <td colspan="3"  align="right"><a href="template/detailbrg_incoming-outgoing.php" target="_self" alt="Buat Template Excel" title="Buat template dalam format excel" ><img src="images/excel.jpg" width="15" height="15"  border="0"  alt="Buat Template Excel" />Buat Template</a></td>
    </tr>
</table>
 ...
 <p>Silakan Pilih File Excel :    <input name="userfile" type="file">
  ...
  <p>
(<span class="style2">klik <strong><a href="template/detailbrg_incoming-outgoing.php" target="_self">Buat Template</a></strong> untuk membuat file template dari excel</span>)  </p>
<table class="table-list" width="100%" border="0" cellspacing="1" cellpadding="2">
  ...
  <tr>
    ...
    <td width="35" align="right" bgcolor="#CCCCCC"><strong>Jumlah</strong> </td>
    <td width="150" align="left" bgcolor="#CCCCCC"><strong>Keterangan</strong> </td>
    <td width="20" align="center" bgcolor="#FFCC00"><b>Edit</b></td>
    <td width="20" align="center" bgcolor="#FFCC00"><b>Del</b></td>
  </tr>
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_detail_incoming_outgoing.*
		FROM barang, tmp_detail_incoming_outgoing
		WHERE tmp_detail_incoming_outgoing.kode=barang.kd_barang
		ORDER BY tmp_detail_incoming_outgoing.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$jml	 = mysql_num_rows($tmpQry);
$dataSql ="SELECT * FROM tmp_detail_incoming_outgoing ";
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Trans-Incoming-Outgoing&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Upload-Detail-Incoming-Outgoing&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
  <tr bgcolor="#FFFFCC">
