...
for ($i=2; $i<=$baris; $i++)
{
  ...
  $query = "INSERT INTO tmp_stock_pengeluaran VALUES ('$id','$kode', '$jumlah')";
  ...
  if ($hasil) $sukses++;
				...
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Upload-Excel-Pengeluaran-Awal'>";
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_stock_pengeluaran WHERE id='".$_GET['ID']."'", $koneksidb) 
	...
	$message = array();
	$cekSql ="SELECT COUNT(kode) As qty FROM tmp_stock_pengeluaran";
		...
		if ($cekRow['qty'] < 1) {
			...
			# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_stock_pengeluaran ";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel pengeluaran awal
	$itemSql = "INSERT INTO stock_pengeluaran SET kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]'";
					mysql_query($itemSql, $koneksidb) or die ("Gagal Query data pengeluaran: ".mysql_error());
						}
						
						# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("TRUNCATE TABLE tmp_stock_pengeluaran ", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Pengeluaran-Stock'>";
						}
		}
...
<form action="?page=Upload-Excel-Pengeluaran-Awal" method="post"  name="frmadd" enctype="multipart/form-data" target="_self">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <th colspan="3">UPLOAD DATA BARANG DARI EXCEL UNTUK STOCK PENGELUARAN AWAL</th>
	</tr>
</table>
 ...
 <p>Silakan Pilih File Excel :    <input name="userfile" type="file">
  ...
  <p>
(cari file excel: <span class="style1">template_stock_pengeluaran.xls</span> di folder template excel)    
  </p>
...
$tmpSql ="SELECT T1.*, T2.*, T3.kd_barang,T3.nm_barang,T3.satuan,T3.kd_kategori
		FROM tmp_stock_pengeluaran as T1, kategori as T2, barang as T3
		WHERE T1.kode=T3.kd_barang and T3.kd_kategori=T2.kd_kategori
		ORDER BY T1.kode ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$jml	 = mysql_num_rows($tmpQry);
$dataSql ="SELECT kode FROM tmp_stock_pengeluaran ";
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Upload-Excel-Pengeluaran-Awal&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
<tr bgcolor="#FFFFCC">
