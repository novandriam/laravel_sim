...
header('Content-Disposition: attachment;filename="laporan_pengeluaran-perjenisdokpab"'.date("dmy").'".xls"');
...
echo "<tr>
  ...
  <tr>
  <td bgcolor=\"#99CCFF\"><div align=\"left\">LAPORAN PENGELUARAN BARANG PER JENIS DOKUMEN PABEAN</div></td>
  </tr>
  <tr>
...
echo "<table width=\"458\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\">
  ...
  <td rowspan=\"2\" ><div align=\"left\">Penerima barang</div></td>
  ...
  ";
	...
	$query="SELECT T1.kd_barang, T1.nm_barang, T1.satuan, T2.nomor, T2.no_sj, T2.tgl_sj, T2.penerima, T2.jenis_dokpab, T2.no_dokpab, T2.tgl_dokpab, T3.nomor, T3.kode, T3.jumlah, T3.valuta, T3.nilai, T4.kode_penerima, T4.nama 
	...
	WHERE T2.jenis_dokpab = '$Jenisdokpab'
	AND T2.tgl_dokpab BETWEEN '".InggrisTgl($_POST['cmbTglStart'])."' AND '".InggrisTgl($_POST['cmbTglEnd'])."' 
	ORDER BY T2.nomor ASC";
$cari=mysql_query($query) or die ("error paging: ".mysql_error()); //ambil data hgr dari database
$nomor = 0;
