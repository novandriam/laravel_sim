...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE tmp_pengeluaran SET kode='$txtKode', jumlah='$txtJumlah', valuta='$cmbValuta', nilai='$txtNilai', netto_detail='$txtNetto_detail', volume_detail='$txtVolume_detail' WHERE id='".$_POST['txtID']."'") 
	...
	$sqlShow = "SELECT barang.kd_barang, barang.nm_barang, barang.satuan, tmp_pengeluaran.* 
	FROM tmp_pengeluaran inner join barang on tmp_pengeluaran.kode=barang.kd_barang
	WHERE tmp_pengeluaran.id='$IDEdit'";
...
?>
<form action="?page=Edit-Pengeluaran-Barang" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr><input name="txtID" type="hidden" value="<?php echo $dataID; ?>" />
	  <td colspan="3"><h2>UBAH  DETAIL DATA PENGELUARAN BARANG </h2></td>
	</tr>
