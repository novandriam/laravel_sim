...
$tahun = "SPB-$today";
$id = 'SPB-';
$query = "SELECT max(no_spb) AS last FROM outgoing_incoming WHERE no_spb LIKE '$tahun%'";
...
if($_GET) {
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_outgoing_incoming WHERE id='".$_GET['ID']."' AND userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) 
	...
	if(isset($_POST['btnDelete'])){
				...
				mysql_query("DELETE FROM tmp_outgoing_incoming WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Outgoing-Incoming-Add&Act=DeleteAllSuccess'>";
		}
		if(isset($_POST['btnExcel'])){
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Upload-Detail-Outgoing-Incoming'>";
	...
	if(isset($_POST['btnPilih'])){
		...
		if(count($message)==0){			
			$barangSql ="SELECT kd_barang FROM barang WHERE kd_barang='$txtKode' or kd_barang='$txtKode' And stok > $txtJumlah";
			...
			if ($barangQty >= 1) {
				...
				$tmpSql = "INSERT INTO tmp_outgoing_incoming SET kode='$txtKode', jumlah='$txtJumlah', ket='$txtKet', userid='".$_SESSION['SES_LOGIN']."'";
	...
	if(isset($_POST['btnSave'])){
		...
		$tmpSql ="SELECT COUNT(id) As qty FROM tmp_outgoing_incoming WHERE userid='".$_SESSION['SES_LOGIN']."'";
		...
		$sqlCeknomor="SELECT nomor,no_spb,tgl_spb FROM outgoing_incoming WHERE nomor='$kodeBaru' or no_spb='$txtSpb' and tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."'";
		$qryCeknomor=mysql_query($sqlCeknomor, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryCeknomor)>=1){
			$message[] = "Maaf, data dengan ID Record<b> $kodeBaru </b> atau Nomor SPB <b> $txtSpb </b> tanggal <b> $cmbTglspb </b> sudah ada sudah ada, kembali ke menu utama untuk mendapatkan ID baru";
		}
		# Jika jumlah error message tidak ada
		if(count($message)==0){			
			$kodeBaru	= buatKode("outgoing_incoming", "000");
			$qrySave=mysql_query("INSERT INTO outgoing_incoming SET nomor='$kodeBaru', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', dept='$cmbDept', name_ppc='$txtPpc', catatan='$txtCatatan', userid='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
			if($qrySave){
			// Masukkan data penyimpanan ke transfer 
			$qryTransfer=mysql_query("INSERT INTO tmp_incoming_terima SET nomor='OUT-$kodeBaru', dari='OUTGOING', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', dept='$cmbDept', name_ppc='$txtPpc', catatan='$txtCatatan', user_trt='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
				
				# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_outgoing_incoming WHERE userid='".$_SESSION['SES_LOGIN']."'";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel transfer detail
					$itemSql = "INSERT INTO outgoing_incoming_detail SET nomor='$kodeBaru', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]'";
		  			mysql_query($itemSql, $koneksidb) or die ("Gagal Query Simpan detail barang".mysql_error());
				// Masukkan semua barang yang udah diisi ke tabel transfer detail
					$itemtrtSql = "INSERT INTO tmp_incoming_terima_detail SET nomor='OUT-$kodeBaru', dari='OUTGOING', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]'";
		  			mysql_query($itemtrtSql, $koneksidb) or die ("Gagal Query Simpan detail barang".mysql_error());	
				
				}
				# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("DELETE FROM tmp_outgoing_incoming WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menambah data transaksi outgoing-incoming nomor SPB: $txtSpb dan tanggal $cmbTglspb', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Outgoing-Incoming-Add&Act=Success'>";
...
$nomorTransaksi = buatKode("outgoing_incoming", "000");
...
<form action="?page=Outgoing-Incoming-Add" method="post"  name="frmadd">
<table width="100%" cellspacing="1" class="table-common" style="margin-top:0px;">
	<tr>
	  <td colspan="3" align="right"><h2>Outgoing - Pengiriman barang ke Incoming</h2></td>
	</tr>
	<tr>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_outgoing_incoming.*
		FROM tmp_outgoing_incoming inner join barang on tmp_outgoing_incoming.kode=barang.kd_barang
		WHERE tmp_outgoing_incoming.userid='".$_SESSION['SES_LOGIN']."'
		ORDER BY tmp_outgoing_incoming.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$qtyBrg = 0; $nomor=0;
while($tmpRow = mysql_fetch_array($tmpQry)) {
	$ID		= $tmpRow['id'];
...
?>
  <tr>
    ...
    <td align="left"><?php echo $tmpRow['ket']; ?></td>
     <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Outgoing-Incoming&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Outgoing-Incoming-Add&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
  <tr>
