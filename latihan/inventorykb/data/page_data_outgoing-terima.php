...
function pagination(page){
  ...
  $.ajax({
    url:"data/page_data_outgoing-terima.php",
    data: dataString,
    type:"GET",
...
function loadData(){
  ...
  $.ajax({
    url: "data/page_data_outgoing-terima.php", //file tempat pemrosesan permintaan (request)
    type: "GET",
    data: dataString,
    success:function(data)
    {
...
$(function(){
  ...
  $('#data/data_outgoing-terima tr:even:not(#nav):not(#total)').addClass('even');
  $('#data/data_outgoing-terima tr:odd:not(#nav):not(#total)').addClass('odd');
...
if (isset($_GET['dari']) and !empty($_GET['dari'])){
  $dari = $_GET['dari'];
  $sql = "select nomor,no_lpb,tgl_lpb,dari,no_spb,tgl_spb FROM outgoing_terima where dari like '%$dari%' ORDER BY nomor desc";
}
else if (isset($_GET['nolpb']) and !empty($_GET['nolpb'])){
  $nolpb = $_GET['nolpb'];
  $sql = "select nomor,no_lpb,tgl_lpb,dari,no_spb,tgl_spb FROM outgoing_terima where no_lpb like '%$nolpb%' ORDER BY nomor desc";
}
else if (isset($_GET['nospb']) and !empty($_GET['nospb'])){
  $nospb = $_GET['nospb'];
  $sql = "select nomor,no_lpb,tgl_lpb,dari,no_spb,tgl_spb FROM outgoing_terima where no_spb like '%$nospb%' ORDER BY nomor desc";
}


else{
  $sql = "select nomor,no_lpb,tgl_lpb,dari,no_spb,tgl_spb FROM outgoing_terima ORDER BY nomor desc";
}
...
?>
  ...
  <table width="100%" id="tbloutgoing-terima">
      ...
      <td align="center"><a href="?page=Outgoing-Lpb-View&amp;Kode=<?php echo $Kode; ?>" target="_self" alt="View Data"><img src="images/btn_view.png" width="20" height="20" border="0" /></a></td>
          <td align="center"><a href="nota/outgoing_lpb.php?nomor=<?php echo $Kode; ?>" target="_new" alt="Cetak LPB" onclick="return confirm('ANDA YAKIN AKAN MENCETAK LPB INI ?')"><img src="images/cetak.jpg" width="20" height="20"  border="0"  alt="Cetak LPB" /></a></td></tr>
  		<?php } //end while ?>
