...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		$txtJumlah	= str_replace("'","&acute;",$txtJumlah);
			...
			if ($barangQty == 0) {
			$message[] = "Tidak ada scrap dengan kode <b>$txtKode</b>, silahkan ganti";
			}

		
		# SIMPAN PERUBAHAN DATA, Jika jumlah error message tidak ada, simpan datanya
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE tmp_scrap SET kode='$txtKode', jumlah='$txtJumlah' WHERE id='".$_POST['txtID']."'") 
					   or die ("Gagal query update".mysql_error());
			if($qryUpdate){
				echo "<meta http-equiv='refresh' content='0; url=?page=Scrap-Add'>";
			}
			exit;
		}	
	...
	$sqlShow = "SELECT barang.kd_barang, barang.nm_barang, barang.satuan, tmp_scrap.* 
	FROM tmp_scrap inner join barang on tmp_scrap.kode=barang.kd_barang
	WHERE tmp_scrap.id='$IDEdit'";
...
?>
<form action="?page=Edit-Scrap" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr><input name="txtID" type="hidden" value="<?php echo $dataID; ?>" />
	  <td colspan="3"><h2>UBAH  DETAIL DATA LAPORAN HASIL SCRAP</h2></td>
	</tr>
