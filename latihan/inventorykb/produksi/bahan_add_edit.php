...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE tmp_bahan SET kode='$txtKode', jumlah='$txtJumlah' WHERE id='".$_POST['txtID']."'") 
	...
	$sqlShow = "SELECT barang.kd_barang, barang.nm_barang, barang.satuan, tmp_bahan.* 
	FROM tmp_bahan inner join barang on tmp_bahan.kode=barang.kd_barang
	WHERE tmp_bahan.id='$IDEdit'";
...
?>
<form action="?page=Edit-Bahan" method="post" name="frmedit">
