<html lang="en">
    ...
    <script type="text/javascript">
			...
			$(document).ready(function(){				
				//Mempercantik Button dengan Jquery UI (UNTUK BAHAN BAKU)
				$("#kode-search1").button({icons: {primary: "ui-icon-search"}, text: false });
				$("#submit").button();
				$("#reset").button();
				/* End */
				
				$("#kode-search1").click(function(){
					$("#kode-dialog1").dialog({
						title:"Cari Kode Barang", 
						resizable:false, 
						width:500, 
					...
					$("#tkodebrg1").jqGrid({
					url: 'json1.php', //URL Tujuan Yg Mengenerate data Json nya
					datatype: "json", //Datatype yg di gunakan
					mtype: 'GET',
					colNames: ['Kode barang', 'Nama barang'],
					colModel: [
					...
					caption: '&nbsp;',
					ondblClickRow: function(rowid) {
						var v = $("#tkodebrg1").getRowData(rowid);
						kd_barang = v['kd_barang'];
						nm_barang = v['nm_barang'];
						$("#kd_barang").val(kd_barang);
						$("#txtKode1").val(kd_barang);
						$("#txtNama1").val(nm_barang);
						$("#kode-dialog1").dialog('close');
					}
				      });
				      //$("#pkodebarang_left .navtable").empty();
				      //$("#tkodebarang").navGrid('#pkodebarang',{edit:false, add:false, del:false});
				      $("#tkodebrg1").jqGrid('navGrid','#pkodebrg1',{view:false,edit:false,add:false,del:false});
				      return false;
				});				
				var config = {
				...
				var config = {
					source: "json_auto1.php",					
					select: function(event, ui){
						$("#txtNama1").val(ui.item.nm_barang); // Mengisi Text Box dengan nama barang
						$("#txtKode1").val(ui.item.kd_barang); // Mengisi Text Box dengan kode barang
					},
					minLength: 1
				};
				$("#txtKode1").autocomplete(config);
...
if($_GET) {
	...
	if(isset($_GET['Act'])){
		...
		if(trim($_GET['Act'])=="Delete1"){
			# Hapus Tmp jika datanya sudah dipindah
			mysql_query("DELETE FROM tmp_bahanbaku WHERE id='".$_GET['ID']."' AND userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) 
				or die ("Gagal kosongkan tmp".mysql_error());
		}
		if(trim($_GET['Act'])=="Succes"){
			echo "<b>DATA BERHASIL DISIMPAN</b> <br><br>";
	...
	if(isset($_POST['btnDelete'])){
				...
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Brgjadi_Gabung-Add&Act=DeleteAllSuccess'>";
		}
...
if(isset($_POST['btnDelete1'])){
				# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("DELETE FROM tmp_bahanbaku WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Brgjadi_Gabung-Add&Act=DeleteAllSuccess'>";
		}
	...
	if(isset($_POST['btnPilih'])){
		...
		if(count($message)==0){			
			$barangSql ="SELECT * FROM barang WHERE kd_barang='$txtKode' and kd_kategori='K02' or kd_barang='$txtKode' and kd_kategori='K04'";
			...
			if ($barangQty >= 1) {
				...
				$message[] = "Tidak ada barang jadi atau barang sisa/scrap dengan kode <b>$txtKode</b>, silahkan ganti";
			}
		}

	}
	
		# TOMBOL PILIH (KODE bahan baku) DIKLIK
	if(isset($_POST['btnPilih1'])){
		$message = array();
		if (trim($_POST['txtKode1'])=="") {
			$message[] = "<b>Kode bahan baku belum diisi</b>, masukkan kode barang sesuai yg terdaftar di database!";		
		}
		
		if (trim($_POST['txtJumlah1'])=="" OR ! is_numeric(trim($_POST['txtJumlah1']))) {
			$message[] = "Data <b>Jumlah barang (Qty) belum diisi</b>, silahkan <b>isi dengan angka</b> !";		
		}
		
		# Baca variabel
		$txtKode1	= $_POST['txtKode1'];
		$txtKode1	= str_replace("'","&acute;",$txtKode1);
		$txtJumlah1	= $_POST['txtJumlah1'];
		$txtJumlah1	= str_replace("'","&acute;",$txtJumlah1);
		...
		if(count($message)==0){			
			$bahanSql ="SELECT T1.*,T2.* FROM stock_produksi as T1, barang as T2 WHERE T1.kode='$txtKode1' AND T1.kode = T2.kd_barang and T2.kd_kategori = 'K01'";
			...
			if ($bahanQty >= 1) {
				...
				$tmpbahanSql = "INSERT INTO tmp_bahanbaku SET kode='$txtKode1', jumlah='$txtJumlah1', userid='".$_SESSION['SES_LOGIN']."'";
				mysql_query($tmpbahanSql, $koneksidb) or die ("Gagal Query detail bahan: ".mysql_error());
				$txtKode1= "";
				$txtJumlah1	= "";
				...
				$message[] = "Tidak ada bahan baku/penolong di stock gudang produksi dengan kode <b>$txtKode1</b>, silahkan ganti";
			}
		}
	...
	if(isset($_POST['btnSave'])){
		...
		$tmpbahanSql ="SELECT COUNT(*) As qty FROM tmp_bahanbaku WHERE userid='".$_SESSION['SES_LOGIN']."'";
		$tmpbahanQry = mysql_query($tmpbahanSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
		$tmpbahanRow = mysql_fetch_array($tmpbahanQry);
		if ($tmpbahanRow['qty'] < 1) {
			$message[] = "<b>Item bahan baku/penolong</b> belum ada yang dimasukan, <b>minimal 1 barang</b>.";
		}
	...
	# Ambil semua data bahan baku yang dipilih, berdasarkan user yg login
				$tmpbahanSql ="SELECT * FROM tmp_bahanbaku WHERE userid='".$_SESSION['SES_LOGIN']."'";
				$tmpbahanQry = mysql_query($tmpbahanSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpbahanRow = mysql_fetch_array($tmpbahanQry)) {
					// Masukkan semua barang yang udah diisi ke tabel barangjadi detail
					$itembahanSql = "INSERT INTO bahanbaku_detail SET nomor='$kodeBaru', no_bpb='$txtBpb', tgl_bpb='".InggrisTgl($_POST['cmbTglbpb'])."', kode='$tmpbahanRow[kode]', jumlah='$tmpbahanRow[jumlah]'";
		  			mysql_query($itembahanSql, $koneksidb) or die ("Gagal Query Simpan detail bahanbaku".mysql_error());

					
					// update stok di stock produksi
			$bahanSql ="SELECT * FROM stock_produksi WHERE kode='$tmpbahanRow[kode]'";
			$bahanQry = mysql_query($bahanSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
			$bahanQty = mysql_num_rows($bahanQry);
			if ($bahanQty >= 1) {

					$updatebahanSql = "UPDATE stock_produksi SET jumlah=jumlah - $tmpbahanRow[jumlah] WHERE kode='$tmpbahanRow[kode]'";
		  			mysql_query($updatebahanSql, $koneksidb) or die ("Gagal Query Edit Stok".mysql_error());
			}
				}
				...
				mysql_query("DELETE FROM tmp_bahanbaku WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
					...
					$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menambah data Barang Jadi hasil produksi dengan nomor BHP $txtBpb dan tanggal $cmbTglbpb', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Brgjadi_Gabung-Add&Act=Success'>";
...
<form action="?page=Brgjadi_Gabung-Add" method="post"  name="frmadd">
<table width="100%" cellspacing="1" class="table-common" style="margin-top:0px;">
	<tr>
	  <td colspan="3" align="right"><h2>TAMBAH LAPORAN PRODUKSI (BARANG JADI/SCRAP) BESERTA PEMAKAIAN BAHAN BAKU/PENOLONG</h2></td>
	</tr>
	<tr>
...
<tr>
	...
	<tr><td colspan="3"><div align="left"><span class="style1">Detail barang jadi/scrap</span></div></td>
    </tr>
...
&nbsp;&nbsp;       
Qty :
<input class="angkaC" name="txtJumlah" size="6" maxlength="12" value="1"  onKeyPress="return tabE(this,event)"
...
<table class="table-list" width="100%" border="0" cellspacing="1" cellpadding="2">
  <tr>
    <th colspan="7">DAFTAR  DETAIL BARANG JADI/SCRAP</th>
    </tr>
  <tr>
...
?>
  <tr>
    ...
    <td align="right"><?php echo format_angka($tmpRow['jumlah']); ?></div></td>
     <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Brgjadi2&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Brgjadi_Gabung-Add&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
  <tr>
...
<tr>
      ...
      <td>&nbsp;</td>
<tr>
	  <td colspan="2"><span class="style1">MASUKKAN DATA BAHAN BAKU/PENOLONG YANG DIGUNAKAN DALAM PROSES PRODUKSI</span></td>
	  <td>&nbsp;</td>
<tr>

	  <td colspan="3"><b>Kode Barang </b><b>:</b><b>
<input onKeyPress="return tabE(this,event)" type="text"  name="txtKode1" size="20" id="txtKode1" class="ui-widget ui-widget-content padding ui-corner-all button">
					<button id="kode-search1" class="tombol">&nbsp;</button>
        <input type="text" readonly="readonly" name="txtNama1" size="35" id="txtNama1" class="ui-widget ui-widget-content padding ui-corner-all button">
&nbsp;&nbsp;       
Qty :
<input class="angkaC" name="txtJumlah1" size="6" maxlength="12" value="1"  onKeyPress="return tabE(this,event)"
	  		 onblur="if (value == '') {value = '1'}" 
      		 onfocus="if (value == '1') {value =''}"/>
&nbsp;&nbsp;
<input name="btnPilih1" type="submit" style="cursor:pointer;" value="Add" id="btnPilih1" />  
 <input name="btnDelete1" type="submit" style="cursor:pointer;" value="Delete All" id="btnDelete1" />
      </b></td>
    </tr>
</table>
<table class="table-list" width="100%" border="0" cellspacing="1" cellpadding="2">
  <tr>
    <th colspan="8" align="left">DAFTAR  DETAIL BAHAN BAKU/PENOLONG YG DIGUNAKAN</th>
    </tr>
  <tr>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_bahanbaku.*
		FROM barang, tmp_bahanbaku
		WHERE barang.kd_barang=tmp_bahanbaku.kode AND tmp_bahanbaku.userid='".$_SESSION['SES_LOGIN']."'
		ORDER BY tmp_bahanbaku.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$qtyBrg = 0; $nomor=0;
while($tmpRow = mysql_fetch_array($tmpQry)) {
	$ID		= $tmpRow['id'];
...
?>
  <tr>
    ...
    <td align="left"><?php echo $tmpRow['nm_barang']; ?></td>
     <td align="center"><?php echo $tmpRow['satuan']; ?></td>
    <td align="right"><?php echo format_angka($tmpRow['jumlah']); ?></div></td>
     <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Bahanbaku&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Brgjadi_Gabung-Add&Act=Delete1&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
  <tr>
...
<div id="kode-dialog1" class="" style="display: none; font-size: 10pt">
			<table id="tkodebrg1" class="scroll" cellpadding="0" cellspacing="0"></table> 
			<div id="pkodebrg1" class='ui-widget-header ui-corner-bl ui-corner-br' style="margin-top:0px;"></div>
		</div>
</body>
</html>