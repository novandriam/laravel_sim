...
$tahun = "LB-$today";
$id = 'LB-';
$query = "SELECT max(no_lap) AS last FROM bahan WHERE no_lap LIKE '$tahun%'";
...
if($_GET) {
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_bahan WHERE id='".$_GET['ID']."' AND userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) 
	...
	if($_POST) {
		if(isset($_POST['btnDelete'])){
				...
				mysql_query("DELETE FROM tmp_bahan WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Bahan-Add&Act=DeleteAllSuccess'>";
		}
	...
	if(isset($_POST['btnPilih'])){
		...
		if(count($message)==0){			
			$barangSql ="SELECT kd_barang FROM barang WHERE kd_barang='$txtKode' and kd_kategori='K01'";
			...
			if ($barangQty >= 1) {
				...
				$tmpSql = "INSERT INTO tmp_bahan SET kode='$txtKode', jumlah='$txtJumlah', userid='".$_SESSION['SES_LOGIN']."'";
				...
				$message[] = "Tidak ada bahan baku atau bahan penolong dengan kode <b>$txtKode</b>, silahkan ganti";
			}
		}
	...
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtLap'])=="") {
			$message[] = "Nomor Laporan Pemakaian Bahan belum diisi!";		
		}

		if (trim($_POST['cmbTgllap'])=="") {
			$message[] = "Tanggal Laporan Pemakaian Bahan belum diisi, pilih pada combo !";		
		}
		...
		$tmpSql ="SELECT COUNT(id) As qty FROM tmp_bahan WHERE userid='".$_SESSION['SES_LOGIN']."'";
		...
		$sqlCeknomor="SELECT nomor,no_lap,tgl_lap FROM bahan WHERE nomor='$kodeBaru' or no_lap='$txtLap' and tgl_lap='".InggrisTgl($_POST['cmbTgllap'])."'";
		$qryCeknomor=mysql_query($sqlCeknomor, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryCeknomor)>=1){
			$message[] = "Maaf, data dengan ID Record<b> $kodeBaru </b> atau Nomor Laporan Pemakaian Bahan <b> $txtLap </b> tanggal <b> $cmbTgllap </b> sudah ada, kembali ke menu utama untuk mendapatkan ID baru";
		}
		# Jika jumlah error message tidak ada
		if(count($message)==0){			
			$kodeBaru	= buatKode("bahan", "000");
			$qrySave=mysql_query("INSERT INTO bahan SET nomor='$kodeBaru', no_lap='$txtLap', tgl_lap='".InggrisTgl($_POST['cmbTgllap'])."', name_ppc='$txtPpc', catatan='$txtCatatan', userid='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
			if($qrySave){
				
				# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_bahan WHERE userid='".$_SESSION['SES_LOGIN']."'";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel transfer detail
					$itemSql = "INSERT INTO bahan_detail SET nomor='$kodeBaru', no_lap='$txtLap', tgl_lap='".InggrisTgl($_POST['cmbTgllap'])."', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]'";
			...
			if ($pemasukanQty == 1) {
				...
				# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("DELETE FROM tmp_bahan WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
					$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menambah data pemakaian bahan baku/penolong dengan nomor Laporan Pemakaian Bahan: $txtLap dan tanggal $cmbTgllap', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Bahan-Add&Act=Success'>";
...
$nomorTransaksi = buatKode("bahan", "000");
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_bahan.*
		FROM tmp_bahan inner join barang on tmp_bahan.kode=barang.kd_barang
		WHERE tmp_bahan.userid='".$_SESSION['SES_LOGIN']."'
		ORDER BY tmp_bahan.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$qtyBrg = 0; $nomor=0;
while($tmpRow = mysql_fetch_array($tmpQry)) {
	$ID		= $tmpRow['id'];
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Bahan&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>

    <td align="center" bgcolor="#FFFFCC"><a href="?page=Bahan-Add&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?> 
  <tr>
