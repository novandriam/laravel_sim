...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		$txtJumlah	= str_replace("'","&acute;",$txtJumlah);
			...
			$barangSql ="SELECT kd_barang FROM barang WHERE kd_barang='$txtKode' and kd_kategori='K02'";
		...
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE tmp_brgjadi SET kode='$txtKode', jumlah='$txtJumlah' WHERE id='".$_POST['txtID']."'") 
	...
	$sqlShow = "SELECT barang.kd_barang, barang.nm_barang, barang.satuan, tmp_brgjadi.* 
	FROM tmp_brgjadi inner join barang on tmp_brgjadi.kode=barang.kd_barang
	WHERE tmp_brgjadi.id='$IDEdit'";
...
?>
<form action="?page=Edit-Brgjadi2" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr><input name="txtID" type="hidden" value="<?php echo $dataID; ?>" />
	  <td colspan="3"><h2>UBAH  DETAIL DATA LAPORAN PRODUKSI (BARANG JADI)</h2></td>
	</tr>
