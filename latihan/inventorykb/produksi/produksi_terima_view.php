...
$tahun = "LPB-$today";
$id = 'LPB-';
$query = "SELECT max(no_lpb) AS last FROM produksi_terima WHERE no_lpb LIKE '$tahun%'";
...
if($_GET) {
	...
	$sqlShow = "SELECT * FROM tmp_produksi_terima WHERE nomor='$KodeView'";
	...
	if(isset($_POST['btnSave'])){
		...
		$sqlCek="SELECT * FROM produksi_terima WHERE nomor='$kodeBaru'";
		...
		$sqlCek="SELECT * FROM produksi_terima WHERE no_lpb='$txtLpb' and tgl_lpb='".InggrisTgl($_POST['cmbTgllpb'])."'";
		$qryCek=mysql_query($sqlCek, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryCek)>=1){
		...
		if(count($message)==0){			
		$kodeBaru	= buatKode("produksi_terima", "000");
			$qrySave=mysql_query("INSERT INTO produksi_terima SET nomor='$kodeBaru', no_lpb='$txtLpb', tgl_lpb='".InggrisTgl($_POST['cmbTgllpb'])."', dari='$txtDari', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['txtTglspb'])."', dept='$txtDept', name_ppc='$txtPpc', catatan='$txtCatatan', user_trt='$txtUser', userid='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
			if($qrySave){				
				# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_produksi_terima_detail WHERE nomor='$txtNomor'";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
				
					// Masukkan semua barang yang udah diisi ke tabel transfer detail
					$itemSql = "INSERT INTO produksi_terima_detail SET nomor='$kodeBaru', dari='$tmpRow[dari]', no_lpb='$txtLpb', tgl_lpb='".InggrisTgl($_POST['cmbTgllpb'])."', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]'"; 
			...
			$terimaSql ="SELECT * FROM tmp_produksi_terima WHERE nomor='$tmpRow[nomor]' and dari='INCOMING'";
			...
			if ($terimaQty == 1) {
				...
				mysql_query("DELETE FROM tmp_produksi_terima WHERE nomor='$txtNomor'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				mysql_query("DELETE FROM tmp_produksi_terima_detail WHERE nomor='$txtNomor'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menerima barang PRODUKSI dengan nomor LPB: $txtLpb dan tanggal $cmbTgllpb', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());


				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Produksi-Terima-Data&Act=Success'>";
...
$nomorTransaksi = buatKode("produksi_terima", "000");
...
<form action="?page=Produksi-Terima-View&Act=Update" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>DATA PENERIMAAN BARANG (PRODUKSI) </h2></td>
	</tr>
   <tr>
...
<tr>
     ...
     <tr><td>&nbsp;</td>
	  <td>&nbsp;</td>
	  <td>
&nbsp;&nbsp;&nbsp;<input type="button" value="Batal" onclick="self.history.back()" />&nbsp;&nbsp;&nbsp; <input name="btnSave" type="submit" style="cursor:pointer;" value="Terima Barang" /></td>
    </tr>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_produksi_terima_detail.*
		FROM tmp_produksi_terima_detail inner join barang on tmp_produksi_terima_detail.kode=barang.kd_barang
		WHERE tmp_produksi_terima_detail.nomor='$KodeView'
		ORDER BY tmp_produksi_terima_detail.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$qtyBrg = 0; $nomor=0;
while($tmpRow = mysql_fetch_array($tmpQry)) {
