...
$tahun = "SPB-$today";
$id = 'SPB-';
$query = "SELECT max(no_spb) AS last FROM produksi_outgoing WHERE no_spb LIKE '$tahun%'";
...
if($_GET) {
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_produksi_outgoing WHERE id='".$_GET['ID']."' AND userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) 
	...
	if($_POST) {
		if(isset($_POST['btnDelete'])){
				...
				mysql_query("DELETE FROM tmp_produksi_outgoing WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Produksi-Outgoing-Add&Act=DeleteAllSuccess'>";
		}
	if(isset($_POST['btnExcel'])){
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Upload-Detail-Produksi-Outgoing'>";
	...
	if(isset($_POST['btnPilih'])){
		...
		# Jika jumlah stok barang di tabel barang tidak cukup, maka transaksi ditolak
		$sqlCekstok="SELECT kd_barang,stok,kd_kategori FROM barang WHERE kd_barang='$txtKode' And stok < $txtJumlah and not kd_kategori='K02' and not kd_kategori='K04'";
		$qryCekstok=mysql_query($sqlCekstok, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryCekstok)>=1){
			$message[] = "Maaf, stok barang kode <b>$txtKode</b> jumlah <b>$txtJumlah</b> di saldo barang tidak cukup";
			
		}
		# Jika jumlah stok barang di tabel gudang produksi tidak cukup, maka transaksi ditolak
		$sqlCek="SELECT * FROM stock_produksi WHERE kode='$txtKode' And jumlah < $txtJumlah";
		$qryCek=mysql_query($sqlCek, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryCek)>=1){
			$message[] = "Maaf, stok barang kode <b>$txtKode</b> jumlah <b>$txtJumlah</b> di saldo gudang produksi tidak cukup";
			
		}
		
		# Jika kode barang tidak ada di tabel gudang produksi, maka transaksi ditolak
		$sqlCek="SELECT kode FROM stock_produksi WHERE kode='$txtKode'";
		$qryCek=mysql_query($sqlCek, $koneksidb) or die ("Eror Query".mysql_error()); 
		$qryRow = mysql_fetch_array($qryCek);
		if(mysql_num_rows($qryCek)==0){
			$message[] = "Tidak ada barang dengan kode <b>$txtKode</b> di gudang produksi, silahkan ganti";
		...
		if(count($message)==0){			
			...
			if ($barangQty >= 1) {
				...
				$tmpSql = "INSERT INTO tmp_produksi_outgoing SET kode='$txtKode', jumlah='$txtJumlah', ket='$txtKet', userid='".$_SESSION['SES_LOGIN']."'";
	...
	if(isset($_POST['btnSave'])){
		...
		$tmpSql ="SELECT COUNT(id) As qty FROM tmp_produksi_outgoing WHERE userid='".$_SESSION['SES_LOGIN']."'";
		...
		$sqlCeknomor="SELECT nomor,no_spb,tgl_spb FROM produksi_outgoing WHERE nomor='$kodeBaru' or no_spb='$txtSpb' and tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."'";
		...
		# Jika jumlah error message tidak ada
		if(count($message)==0){			
			$kodeBaru	= buatKode("produksi_outgoing", "000");
			$qrySave=mysql_query("INSERT INTO produksi_outgoing SET nomor='$kodeBaru', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', dept='$cmbDept', name_ppc='$txtPpc', catatan='$txtCatatan', userid='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
			if($qrySave){
			// Masukkan data penyimpanan ke transfer 
			$qryTransfer=mysql_query("INSERT INTO tmp_outgoing_terima SET nomor='PROD-$kodeBaru', dari='PRODUKSI', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', dept='$cmbDept', name_ppc='$txtPpc', catatan='$txtCatatan', user_trt='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
				
				# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_produksi_outgoing WHERE userid='".$_SESSION['SES_LOGIN']."'";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel transfer detail
					$itemSql = "INSERT INTO produksi_outgoing_detail SET nomor='$kodeBaru', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]'";
		  			mysql_query($itemSql, $koneksidb) or die ("Gagal Query Simpan detail barang".mysql_error());
				// Masukkan semua barang yang udah diisi ke tabel transfer detail
					$itemtrtSql = "INSERT INTO tmp_outgoing_terima_detail SET nomor='PROD-$kodeBaru', dari='PRODUKSI', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]'";
		  			mysql_query($itemtrtSql, $koneksidb) or die ("Gagal Query Simpan detail barang".mysql_error());	
				
				}
				# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("DELETE FROM tmp_produksi_outgoing WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
					$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menambah data transaksi pemasukan barang dengan nomor SPB: $txtSpb dan tanggal $cmbTglspb', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Produksi-Outgoing-Add&Act=Success'>";
...
$nomorTransaksi = buatKode("produksi_outgoing", "000");
...
<form action="?page=Produksi-Outgoing-Add" method="post"  name="frmadd">
<table width="100%" cellspacing="1" class="table-common" style="margin-top:0px;">
	<tr>
	  <td colspan="3" align="right"><h2>Produksi - Pengiriman barang ke Outgoing</h2></td>
	</tr>
	<tr>
	...
	<tr>
	  ...
	  <td width="1%"><b>:</b></td>
	  <td width="69%"><input name="txtSpb" readonly="readonly" onKeyPress="return tabE(this,event)" value="<?php echo $nextNoTransaksi; ?>" size="45" maxlength="100" /></td></tr>
...
<tr>
      <td><b>Nama PIC</b></td>
	  <td><b>:</b></td>
	  <td><input name="txtPpc" value="<?php echo $dataPpc; ?>" size="50" onKeyPress="return tabE(this,event)" maxlength="100" /></td>
    </tr>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_produksi_outgoing.*
		FROM tmp_produksi_outgoing inner join barang on tmp_produksi_outgoing.kode=barang.kd_barang
		WHERE tmp_produksi_outgoing.userid='".$_SESSION['SES_LOGIN']."'
		ORDER BY tmp_produksi_outgoing.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$qtyBrg = 0; $nomor=0;
while($tmpRow = mysql_fetch_array($tmpQry)) {
	$ID		= $tmpRow['id'];
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Produksi-Outgoing&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Produksi-Outgoing-Add&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?> 
  <tr>
