...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if(mysql_num_rows($qryCek)==0){
			...
			if ($barangQty == 0) {
			$message[] = "Tidak ada bahan baku dengan kode <b>$txtKode</b>, silahkan ganti";
			}

		
		# SIMPAN PERUBAHAN DATA, Jika jumlah error message tidak ada, simpan datanya
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE tmp_bahanbaku SET kode='$txtKode', jumlah='$txtJumlah' WHERE id='".$_POST['txtID']."'") 
					   or die ("Gagal query update".mysql_error());
			if($qryUpdate){
				echo "<meta http-equiv='refresh' content='0; url=?page=Brgjadi_Gabung-Add'>";
			}
			exit;
		}	
	...
	$sqlShow = "SELECT barang.kd_barang, barang.nm_barang, barang.satuan, tmp_bahanbaku.* 
	FROM tmp_bahanbaku inner join barang on tmp_bahanbaku.kode=barang.kd_barang
	WHERE tmp_bahanbaku.id='$IDEdit'";
...
?>
<form action="?page=Edit-Bahanbaku" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr><input name="txtID" type="hidden" value="<?php echo $dataID; ?>" />
	  <td colspan="3"><h2>UBAH  DETAIL DATA LAPORAN PEMAKAIAN BAHAN BAKU/BAHAN PENOLONG</h2></td>
	</tr>
