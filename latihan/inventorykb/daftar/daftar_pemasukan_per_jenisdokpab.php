...
if($_GET) {
	...
	if (isset($_POST['btnShow'])) {
		$SqlPeriode = "AND ( T2.jenis_dokpab='".$_POST['cmbDokpab']."' AND T2.tgl_dokpab BETWEEN '".InggrisTgl($_POST['cmbTglStart'])."' AND '".InggrisTgl($_POST['cmbTglEnd'])."')";
	}
	else {
		$startTgl 	= isset($_GET['startTgl']) ? $_GET['startTgl'] : $tglStart;
		$endTgl 	= isset($_GET['endTgl']) ? $_GET['endTgl'] : $tglEnd;
		$cmbJenisdokpab 	= isset($_GET['cmbDokpab']) ? $_GET['cmbDokpab'] : $Jenisdokpab;
...
$max	 = ceil($jml/$row);
?>
<h2>LAPORAN PEMASUKAN BARANG PER JENIS DOKUMEN PABEAN</h2>
<form action="?page=Lap-Pemasukan-Perjenisdokpab" method="post" name="form1" target="_self" id="form1">
...
<tr>
  ...
  <tr>
	...
	for ($h = 1; $h <= $max; $h++) {
		$list[$h] = $row * $h - $row;
		echo " <a href='?page=Lap-Pemasukan-Perjenisdokpab&hal=$list[$h]&startTgl='>$h</a> ";
