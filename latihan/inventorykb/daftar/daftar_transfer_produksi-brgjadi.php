...
$pageSql = "SELECT T1.kd_barang,T1.nm_barang,T1.satuan, T2.nomor,T2.no_bpb,T2.tgl_bpb,T2.name_ppc, T3.nomor, T3.kode,T3.jumlah 
FROM brgjadi As T2 inner join brgjadi_detail As T3 on T2.nomor = T3.nomor inner join barang As T1 on T3.kode = T1.kd_barang
			 WHERE $SqlPeriode
";
...
$max	 = ceil($jml/$row);
?>
<h2>LAPORAN PENGIRIMAN BRG JADI  - OUTGOING</h2>
<form action="?page=Lap-Pengiriman-Produksi-Brgjadi" method="post" name="form1" target="_self" id="form1">
...
<tr>
  ...
  <td width="77" bgcolor="#CCCCCC" align="center" >Tanggal</td>
	...
	$dataSql = "SELECT T1.kd_barang,T1.nm_barang,T1.satuan, T2.nomor,T2.no_bpb,T2.tgl_bpb,T2.name_ppc, T3.nomor, T3.kode,T3.jumlah	 FROM brgjadi As T2 inner join brgjadi_detail As T3 on T2.nomor = T3.nomor inner join barang As T1 on T3.kode = T1.kd_barang
		 WHERE $SqlPeriode ORDER BY T2.nomor ASC LIMIT $hal, $row";
  ...
  <tr>
	...
	for ($h = 1; $h <= $max; $h++) {
		$list[$h] = $row * $h - $row;
		echo " <a href='?page=Lap-Pengiriman-Produksi-Brgjadi&hal=$list[$h]&startTgl='>$h</a> ";
	}
	?></td>
  </tr>
</table>
