...
FROM produksi_outgoing As T2 inner join produksi_outgoing_detail As T3 on T2.nomor = T3.nomor inner join barang As T1 on T3.kode = T1.kd_barang WHERE $SqlPeriode";
$pageQry = mysql_query($pageSql, $koneksidb) or die ("error paging: ".mysql_error());
$jml	 = mysql_num_rows($pageQry);
$max	 = ceil($jml/$row);
?>
<h2>LAPORAN PENGIRIMAN PRODUKSI - OUTGOING</h2>
<form action="?page=Lap-Pengiriman-Produksi-Outgoing" method="post" name="form1" target="_self" id="form1">
...
<tr>
  ...
  <tr>
	...
	for ($h = 1; $h <= $max; $h++) {
		$list[$h] = $row * $h - $row;
		echo " <a href='?page=Lap-Pengiriman-Produksi-Outgoing&hal=$list[$h]&startTgl='>$h</a> ";
	}
	?></td>
  </tr>
</table>
