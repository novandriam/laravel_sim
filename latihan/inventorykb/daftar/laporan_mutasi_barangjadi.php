...
<h2>LAPORAN PERTANGGUNGJAWABAN MUTASI BARANG JADI</h2>
<form id="form1" name="form1" method="post" action="">
  <table width="100%" border="0"  class="table-list">
    ...
    <tr><td colspan="3"><input name="txtKategori" value="K02" type="hidden" size="10" maxlength="10" readonly="readonly"/></td></tr>
...
<tr>
 ...
 <td bgcolor="#CCCCCC" align="center"><?php echo $dataTglstock; ?></td>
  ...
  echo "<form method='post' target='_new' action='view/excel_mutasi_barangjadi.php'>";
  while($rbrg = mysql_fetch_array($brg)){
             ...
             //cari pemasukan dari pemasok
              ...
              $hasilproduksi = mysql_query(" SELECT sum( jumlah ) AS jml FROM outgoing_terima_detail
			...
			$total_pemasukan = $rpemasukan[jml]+$rhasilproduksi[jml];
              ...
              $outgoingreturn = mysql_query(" SELECT sum( jumlah ) AS jml FROM produksi_terima_detail
                                           WHERE tgl_lpb between '".InggrisTgl($_POST[tgl])."' and '".InggrisTgl($_POST[tglakhir])."' AND dari='OUTGOING' AND kode = '$rbrg[kd_barang]'")or die ("Gagal query".mysql_error());
			...
			$total_pengeluaran = $rpengeluaran[jml]+$routgoingreturn[jml];
			  ...
			  echo "<input type='hidden' name='tgl' value='$tgl'>
				   ...
				   <input type='hidden' name='pemasukan$no' value='$total_pemasukan'>
				   <input type='hidden' name='pengeluaran$no' value='$total_pengeluaran'>
