...
$Penerima	= isset($_POST['cmbPenerima']) ? $_POST['cmbPenerima'] : '';
...
if($_GET) {
	...
	if (isset($_POST['btnShow'])) {
		$SqlPeriode = "AND ( T2.tgl_sj BETWEEN '".InggrisTgl($_POST['cmbTglStart'])."' AND '".InggrisTgl($_POST['cmbTglEnd'])."')";
	}
	else {
		$startTgl 	= isset($_GET['startTgl']) ? $_GET['startTgl'] : $tglStart;
		$endTgl 	= isset($_GET['endTgl']) ? $_GET['endTgl'] : $tglEnd;
		$cmbPenerima 	= isset($_GET['cmbPenerima']) ? $_GET['cmbPenerima'] : $Penerima;
				
		$SqlPeriode = " AND (T2.penerima='$Penerima' AND T2.tgl_sj BETWEEN '".InggrisTgl($startTgl)."' AND '".InggrisTgl($endTgl)."')";
	}
}
...
$pageSql = "SELECT T1.kd_barang,T1.nm_barang,T1.satuan, T2.nomor,T2.no_sj,T2.tgl_sj,T2.penerima,T2.jenis_dokpab,T2.no_dokpab,T2.tgl_dokpab, T3.nomor,T3.kode,T3.jumlah,T3.valuta,T3.nilai, T4.kode_penerima,T4.nama FROM barang As T1, pengeluaran As T2, pengeluaran_detail As T3, penerima as T4
			 ...
			 AND T2.penerima = '$Penerima'
			 AND T2.penerima = T4.kode_penerima
...
$max	 = ceil($jml/$row);
?>
<h2>LAPORAN PENGELUARAN BARANG PER NAMA PENERIMA</h2>
<form action="?page=Lap-Pengeluaran-Perpenerima" method="post" name="form1" target="_self" id="form1">
  <table width="100%" border="0"  class="table-list">
    <tr>
      <td colspan="3" bgcolor="#CCCCCC"><div align="left"><strong>PENERIMA DAN PERIODE TANGGAL PENGELUARAN</strong></div></td>
    </tr>
    <tr>
      <td><b>Penerima</b></td>
	  <td><b>:</b></td>
...
<table class="table-list" width="100%" border="1" cellspacing="1" cellpadding="2">
  <tr>
    ...
    <td align="left" rowspan="2" width="128" bgcolor="#CCCCCC"><b>Penerima brg</b></td>
...
<tr>
  ...
  <tr>
	...
	for ($h = 1; $h <= $max; $h++) {
		$list[$h] = $row * $h - $row;
		echo " <a href='?page=Lap-Pengeluaran-Perpenerima&hal=$list[$h]&startTgl='>$h</a> ";
