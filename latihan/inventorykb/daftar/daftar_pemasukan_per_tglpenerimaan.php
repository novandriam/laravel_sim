...
if($_GET) {
	if (isset($_POST['btnShow'])) {
		$SqlPeriode = "T2.tgl_bpb BETWEEN '".InggrisTgl($_POST['cmbTglStart'])."' AND '".InggrisTgl($_POST['cmbTglEnd'])."'";
	}
	else {
		$startTgl 	= isset($_GET['startTgl']) ? $_GET['startTgl'] : $tglStart;
		$endTgl 	= isset($_GET['endTgl']) ? $_GET['endTgl'] : $tglEnd; 
		$SqlPeriode = "T2.tgl_bpb BETWEEN '".InggrisTgl($startTgl)."' AND '".InggrisTgl($endTgl)."'";
	}
}
...
$pageSql = "SELECT T1.kd_barang, T1.nm_barang, T1.satuan, T2.nomor, T2.no_bpb, T2.tgl_bpb, T2.pemasok, T2.jenis_dokpab, T2.no_dokpab, T2.tgl_dokpab, T3.nomor, T3.kode, T3.jumlah, T3.valuta, T3.nilai, T4.kode_pemasok, T4.nama 
...
$max	 = ceil($jml/$row);
?>
<h2>LAPORAN PEMASUKAN BARANG PER TANGGAL BPB</h2>
<form action="?page=Lap-Pemasukan-Pertglpenerimaan" method="post" name="form1" target="_self" id="form1">
  <table width="100%" border="0"  class="table-list">
    <tr>
      <td colspan="3" bgcolor="#CCCCCC"><div align="left"><strong>PERIODE TANGGAL BPB</strong></div></td>
    </tr>
    <tr>
...
<table class="table-list" width="100%" border="1" cellspacing="1" cellpadding="2">
  <tr>
    ...
    <td colspan="3" align="center" bgcolor="#CCCCCC"><strong>Dokumen Pabean</strong></td>
    <td colspan="2" align="center" bgcolor="#CCCCCC"><b>BPB</b></td>
    <td align="left" rowspan="2" width="208" bgcolor="#CCCCCC"><b>Pemasok/Pengirim </b></td>
    <td align="left" rowspan="2" width="129" bgcolor="#CCCCCC"><b>Kode barang </b></td>
    <td align="left" rowspan="2" width="201" bgcolor="#CCCCCC"><b>Nama barang </b></td>
    ...
    <td rowspan="2" colspan="2" width="86" align="right" bgcolor="#CCCCCC"><b>Nilai barang</b></td>  
<tr>
  <td width="59" bgcolor="#CCCCCC" align="center">Jenis</td>
  <td width="64" bgcolor="#CCCCCC" align="center">Nomor</td>
  ...
  <td width="77" bgcolor="#CCCCCC" align="center" >Tanggal</td>
	...
	$dataSql = "SELECT T1.kd_barang, T1.nm_barang, T1.satuan, T2.nomor, T2.no_bpb, T2.tgl_bpb, T2.pemasok, T2.jenis_dokpab, T2.no_dokpab, T2.tgl_dokpab, T3.nomor, T3.kode, T3.jumlah, T3.valuta, T3.nilai, T4.kode_pemasok, T4.nama 
	FROM pemasukan As T2 inner join pemasukan_detail As T3 on T2.nomor = T3.nomor inner join pemasok as T4 on T2.pemasok = T4.kode_pemasok inner join barang As T1 on T3.kode = T1.kd_barang
		 WHERE $SqlPeriode
  ...
  <tr>
	...
	for ($h = 1; $h <= $max; $h++) {
		$list[$h] = $row * $h - $row;
		echo " <a href='?page=Lap-Pemasukan-Pertglpenerimaan&hal=$list[$h]&startTgl='>$h</a> ";
	}
	?></td>
  </tr>
</table>
