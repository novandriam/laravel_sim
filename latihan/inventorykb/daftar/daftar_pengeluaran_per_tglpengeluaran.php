...
if($_GET) {
	if (isset($_POST['btnShow'])) {
		$SqlPeriode = "T2.tgl_sj BETWEEN '".InggrisTgl($_POST['cmbTglStart'])."' AND '".InggrisTgl($_POST['cmbTglEnd'])."'";
	}
	else {
		$startTgl 	= isset($_GET['startTgl']) ? $_GET['startTgl'] : $tglStart;
		$endTgl 	= isset($_GET['endTgl']) ? $_GET['endTgl'] : $tglEnd; 
		$SqlPeriode = "T2.tgl_sj BETWEEN '".InggrisTgl($startTgl)."' AND '".InggrisTgl($endTgl)."'";
	}
}
...
$max	 = ceil($jml/$row);
?>
<h2>LAPORAN PENGELUARAN BARANG PER TANGGAL SURAT JALAN</h2>
<form action="?page=Lap-Pengeluaran-Pertglpengeluaran" method="post" name="form1" target="_self" id="form1">
  <table width="100%" border="0"  class="table-list">
    <tr>
      <td colspan="3" bgcolor="#CCCCCC"><div align="left"><strong>PERIODE TANGGAL SURAT JALAN</strong></div></td>
    </tr>
    <tr>
...
<tr>
  ...
  <tr>
	...
	for ($h = 1; $h <= $max; $h++) {
		$list[$h] = $row * $h - $row;
		echo " <a href='?page=Lap-Pengeluaran-Pertglpengeluaran&hal=$list[$h]&startTgl='>$h</a> ";
	}
	?></td>
  </tr>
</table>
