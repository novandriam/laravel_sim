...
FROM produksi_terima As T2 inner join produksi_terima_detail As T3 on T2.nomor = T3.nomor inner join barang as T1 on T3.kode = T1.kd_barang
WHERE $SqlPeriode	 	
";
...
$max	 = ceil($jml/$row);
?>
<h2>LAPORAN PENERIMAAN BARANG (PRODUKSI) </h2>
<form action="?page=Lap-Produksi-Terima" method="post" name="form1" target="_self" id="form1">
...
<table class="table-list" width="100%" border="1" cellspacing="1" cellpadding="2">
  <tr>
    ...
    <td align="left" rowspan="2" width="90" bgcolor="#CCCCCC"><b>Dari </b></td>
 ...
 <tr>
  <td width="150" bgcolor="#CCCCCC" align="left">Nomor</td>
  ...
  <td width="77" bgcolor="#CCCCCC" align="center" >Tanggal</td>
	...
	FROM produksi_terima As T2 inner join produksi_terima_detail As T3 on T2.nomor = T3.nomor inner join barang as T1 on T3.kode = T1.kd_barang WHERE $SqlPeriode ORDER BY T2.nomor ASC LIMIT $hal, $row";
  ...
  <tr>
	...
	for ($h = 1; $h <= $max; $h++) {
		$list[$h] = $row * $h - $row;
		echo " <a href='?page=Lap-Produksi-Terima&hal=$list[$h]&startTgl='>$h</a> ";
	}
	?></td>
  </tr>
</table>
