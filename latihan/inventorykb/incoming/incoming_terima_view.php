...
$tahun = "LPB-$today";
$id = 'LPB-';
$query = "SELECT max(no_lpb) AS last FROM incoming_terima WHERE no_lpb LIKE '$tahun%'";
...
if($_GET) {
	...
	$sqlShow = "SELECT * FROM tmp_incoming_terima WHERE nomor='$KodeView'";
	...
	if(isset($_POST['btnSave'])){
		...
		$sqlCek="SELECT * FROM incoming_terima WHERE nomor='$kodeBaru'";
		...
		$sqlCek="SELECT * FROM incoming_terima WHERE no_lpb='$txtLpb' and tgl_lpb='".InggrisTgl($_POST['cmbTgllpb'])."'";
		$qryCek=mysql_query($sqlCek, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryCek)>=1){
		...
		if(count($message)==0){			
		$kodeBaru	= buatKode("incoming_terima", "000");
			$qrySave=mysql_query("INSERT INTO incoming_terima SET nomor='$kodeBaru', no_lpb='$txtLpb', tgl_lpb='".InggrisTgl($_POST['cmbTgllpb'])."', dari='$txtDari', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['txtTglspb'])."', dept='$txtDept', name_ppc='$txtPpc', catatan='$txtCatatan', user_trt='$txtUser', userid='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
			if($qrySave){				
				# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_incoming_terima_detail WHERE nomor='$txtNomor'";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel transfer detail
					$itemSql = "INSERT INTO incoming_terima_detail SET nomor='$kodeBaru', dari='$tmpRow[dari]', no_lpb='$txtLpb', tgl_lpb='".InggrisTgl($_POST['cmbTgllpb'])."', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]'"; 
		  			mysql_query($itemSql, $koneksidb) or die ("Gagal Query Simpan detail barang".mysql_error());
					
				// Update stok saldo barang (mengurangi bahan baku menjadi WIP)
					$barangSql = "UPDATE barang SET stok=stok + $tmpRow[jumlah] WHERE kd_barang='$tmpRow[kode]' and kd_kategori='K01'";
			...
			$terimaSql ="SELECT * FROM tmp_incoming_terima WHERE nomor='$tmpRow[nomor]' and dari='PRODUKSI'";
			...
			if ($terimaQty == 1) {
				...
				mysql_query("DELETE FROM tmp_incoming_terima WHERE nomor='$txtNomor'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				mysql_query("DELETE FROM tmp_incoming_terima_detail WHERE nomor='$txtNomor'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());


				// Refresh form
					$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menerima barang INCOMING dengan nomor LPB: $txtLpb dan tanggal $cmbTgllpb', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
				echo "<meta http-equiv='refresh' content='0; url=?page=Incoming-Terima-Data&Act=Success'>";
...
$nomorTransaksi = buatKode("incoming_terima", "000");
...
<form action="?page=Incoming-Terima-View&Act=Update" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr>
	  <td colspan="3"><h2>DATA PENERIMAAN BARANG (INCOMING)</h2> </td>
	</tr>
   <tr>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_incoming_terima_detail.*
		FROM tmp_incoming_terima_detail inner join barang on tmp_incoming_terima_detail.kode=barang.kd_barang
		WHERE tmp_incoming_terima_detail.nomor='$KodeView'
		ORDER BY tmp_incoming_terima_detail.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$qtyBrg = 0; $nomor=0;
while($tmpRow = mysql_fetch_array($tmpQry)) {
