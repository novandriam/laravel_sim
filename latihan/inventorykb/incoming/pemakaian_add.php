...
$tahun = "SPB-$today";
$id = 'SPB-';
$query = "SELECT max(no_spb) AS last FROM produksi WHERE no_spb LIKE '$tahun%'";
...
if($_GET) {
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_produksi WHERE id='".$_GET['ID']."' AND userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) 
	...
	if(isset($_POST['btnDelete'])){
				...
				mysql_query("DELETE FROM tmp_produksi WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Pemakaian-Add&Act=DeleteAllSuccess'>";
		}
	...
	if(isset($_POST['btnPilih'])){
		...
		if(count($message)==0){			
			...
			if ($barangQty >= 1) {
				...
				$tmpSql = "INSERT INTO tmp_produksi SET kode='$txtKode', jumlah='$txtJumlah', ket='$txtKet', userid='".$_SESSION['SES_LOGIN']."'";
	...
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtNorequest'])=="") {
			$message[] = "Nomor request / order belum diisi!";		
		}
		if (trim($_POST['cmbTglrequest'])=="") {
			$message[] = "Tanggal Request belum diisi, pilih pada combo !";		
		}
		...
		$tmpSql ="SELECT COUNT(id) As qty FROM tmp_produksi WHERE userid='".$_SESSION['SES_LOGIN']."'";
		...
		$txtNorequest= $_POST['txtNorequest'];
		$txtNorequest= str_replace("'","&acute;",$txtNorequest);
		...
		$cmbTglrequest =$_POST['cmbTglrequest'];
		
		# Validasi ID Record, jika sudah ada akan ditolak
		$sqlNomor="SELECT nomor,no_spb,tgl_spb FROM produksi WHERE nomor='$kodeBaru' or no_spb='$txtSpb' and tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."'";
		$qryNomor=mysql_query($sqlNomor, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryNomor)>=1){
		...
		# Jika jumlah error message tidak ada
		if(count($message)==0){			
			$kodeBaru	= buatKode("produksi", "000");
			$qrySave=mysql_query("INSERT INTO produksi SET nomor='$kodeBaru', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', no_request='$txtNorequest', tgl_request='".InggrisTgl($_POST['cmbTglrequest'])."', dept='$cmbDept', name_ppc='$txtPpc', catatan='$txtCatatan', userid='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
			if($qrySave){
			// Masukkan data penyimpanan ke transfer 
			$qryTransfer=mysql_query("INSERT INTO tmp_produksi_terima SET nomor='INC-$kodeBaru', dari='INCOMING', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', dept='$cmbDept', name_ppc='$txtPpc', catatan='$txtCatatan', user_trt='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
				# Ambil semua data barang yang dipilih, berdasarkan user yg login
				$tmpSql ="SELECT * FROM tmp_produksi WHERE userid='".$_SESSION['SES_LOGIN']."'";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel penjualan detail
					$itemSql = "INSERT INTO produksi_detail SET nomor='$kodeBaru', no_spb='$txtSpb', tgl_spb='".InggrisTgl($_POST['cmbTglspb'])."', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]'";
					...
					$itemtrtSql = "INSERT INTO tmp_produksi_terima_detail SET nomor='INC-$kodeBaru', dari='INCOMING', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', ket='$tmpRow[ket]'";
				...
				# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("DELETE FROM tmp_produksi WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
					$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menambah data transaksi incoming-produksi dengan nomor SPB: $txtSpb dan tanggal $cmbTglspb', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Pemakaian-Add&Act=Success'>";
...
$nomorTransaksi = buatKode("produksi", "000");
$dataSpb	= isset($_POST['txtSpb']) ? $_POST['txtSpb'] : '';
$tglSpb		 	= isset($_POST['cmbTglspb']) ? $_POST['cmbTglspb'] : date('d-m-Y');
$tglRequest		= isset($_POST['cmbTglrequest']) ? $_POST['cmbTglrequest'] : date('d-m-Y');
$dataNorequest	= isset($_POST['txtNorequest']) ? $_POST['txtNorequest'] : '';
...
<tr>
      <td><b>No Request/Order</b></td>
	  <td><b>:</b></td>
	  <td><input name="txtNorequest" value="<?php echo $dataNorequest; ?>" size="50" maxlength="100" onKeyPress="return tabE(this,event)" /></td>
    </tr>
<tr>
      <td><b>Tgl Request </b></td>
	  <td><b>:</b></td>
	  <td><?php echo form_tanggal("cmbTglrequest",$tglRequest); ?></td>
    </tr>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_produksi.*
		FROM tmp_produksi inner join barang on tmp_produksi.kode=barang.kd_barang
		WHERE tmp_produksi.userid='".$_SESSION['SES_LOGIN']."'
		ORDER BY tmp_produksi.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
$qtyBrg = 0; $nomor=0;
while($tmpRow = mysql_fetch_array($tmpQry)) {
	$ID		= $tmpRow['id'];
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Pemakaian&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Pemakaian-Add&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
  <tr>
