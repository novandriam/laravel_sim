...
$query = "SELECT max(no_bpb) AS last FROM pemasukan WHERE no_bpb LIKE '$tahun%'";
...
if($_GET) {
	...
	if(isset($_GET['Act'])){
		if(trim($_GET['Act'])=="Delete"){
			...
			mysql_query("DELETE FROM tmp_pemasukan WHERE id='".$_GET['ID']."' AND userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) 
	...
	if(isset($_POST['btnDelete'])){
				...
				mysql_query("DELETE FROM tmp_pemasukan WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Pemasukan-Barang&Act=DeleteAllSuccess'>";
	...
	if(isset($_POST['btnPilih'])){
		...
		if (trim($_POST['txtJumlah'])=="" OR ! is_numeric(trim($_POST['txtJumlah']))) {
			$message[] = "Data <b>Jumlah barang belum diisi</b>!";		
		}
		...
		if(count($message)==0){			
			...
			if ($barangQty >= 1) {
				...
				$tmpSql = "INSERT INTO tmp_pemasukan SET kode='$txtKode', jumlah='$txtJumlah', valuta='$cmbValuta', nilai='$txtNilai', netto_detail='$txtNetto_detail', volume_detail='$txtVolume_detail', userid='".$_SESSION['SES_LOGIN']."'";
	...
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['cmbTgldokpab'])=="") {
			$message[] = "Tanggal Dokumen pabean belum diisi, pilih pada combo !";		
		}
		if (trim($_POST['cmbSupplier'])=="BLANK") {
			$message[] = "<b>Nama pemasok belum dipilih</b>, silahkan pilih lagi !";		
		}
		if (trim($_POST['cmbCatatan'])=="BLANK") {
			$message[] = "<b>Catatan pemasukan barang belum dipilih</b>, silahkan pilih lagi !";		
		}
		...
		if (trim($_POST['cmbTglinvoice'])=="") {
			$message[] = "<b>Tanggal Invoice</b> belum diisi, pilih pada combo !";		
		}
		if (trim($_POST['txtKontrak'])=="") {
			$message[] = "<b>Nomor Kontrak</b> belum diisi!";		
		}
		if (trim($_POST['cmbTglkontrak'])=="") {
			$message[] = "<b>Tanggal kontrak </b>belum diisi, pilih pada combo !";		
		}
		if (trim($_POST['txtDo'])=="") {
			$message[] = "<b>Nomor P/O </b>belum diisi!";		
		}
		...
		if (trim($_POST['txtVolume'])=="") {
			$message[] = "T<b>otal Volume barang </b>belum diisi!";		
		}
		
		$tmpSql ="SELECT COUNT(id) As qty FROM tmp_pemasukan WHERE userid='".$_SESSION['SES_LOGIN']."'";
		...
		$sqlKode="SELECT nomor,no_bpb,tgl_bpb FROM pemasukan WHERE nomor='$kodeBaru' or no_bpb='$txtBpb' and tgl_bpb='".InggrisTgl($_POST['cmbTglbpb'])."'";
		$qryKode=mysql_query($sqlKode, $koneksidb) or die ("Eror Query".mysql_error()); 
		if(mysql_num_rows($qryKode)>=1){
			$message[] = "Maaf, data dengan ID Record<b> $kodeBaru </b> atau Nomor BPB <b> $txtBpb </b> tanggal <b> $cmbTglbpb </b>sudah ada, kembali ke menu utama untuk mendapatkan ID baru";
		}

		# Jika jumlah error message tidak ada
		if(count($message)==0){			
			$kodeBaru	= buatKode("pemasukan", "000");
			$qrySave=mysql_query("INSERT INTO pemasukan SET nomor='$kodeBaru', no_bpb='$txtBpb', tgl_bpb='".InggrisTgl($_POST['cmbTglbpb'])."', pemasok='$cmbSupplier', no_invoice='$txtInvoice', tgl_invoice='".InggrisTgl($_POST['cmbTglinvoice'])."', no_kontrak='$txtKontrak', tgl_kontrak='".InggrisTgl($_POST['cmbTglkontrak'])."', no_do='$txtDo', catatan='$cmbCatatan', jenis_dokpab='$cmbDokpab', no_dokpab='$txtNodokpab', tgl_dokpab='".InggrisTgl($_POST['cmbTgldokpab'])."', harga='$txtHarga', jenis_kemasan='$cmbKemasan', merek_kemasan='$txtMerekKemasan', jumlah_kemasan='$txtJmlKemasan', brutto='$txtBrutto', netto='$txtNetto', volume='$txtVolume', userid='".$_SESSION['SES_LOGIN']."'") or die ("Gagal query".mysql_error());
			if($qrySave){
				# Ambil semua data barang yang dipilih, berdasarkan user yang login
				$tmpSql ="SELECT * FROM tmp_pemasukan WHERE userid='".$_SESSION['SES_LOGIN']."'";
				$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
				while ($tmpRow = mysql_fetch_array($tmpQry)) {
					// Masukkan semua barang yang udah diisi ke tabel penjualan detail
					$itemSql = "INSERT INTO pemasukan_detail SET nomor='$kodeBaru', no_bpb='$txtBpb', tgl_bpb='".InggrisTgl($_POST['cmbTglbpb'])."', kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]', valuta='$tmpRow[valuta]', nilai='$tmpRow[nilai]',  netto_detail='$tmpRow[netto_detail]',  volume_detail='$tmpRow[volume_detail]'";
			...
			// update stok di stock pemasukan
			$pemasukanSql ="SELECT * FROM stock_pemasukan WHERE kode='$tmpRow[kode]'";
			...
			if  ($pemasukanQty == 1) {
				...
				$insertSql = "INSERT INTO stock_pemasukan SET kode='$tmpRow[kode]', jumlah='$tmpRow[jumlah]'";
				...
				# Kosongkan Tmp jika datanya sudah dipindah
				mysql_query("DELETE FROM tmp_pemasukan WHERE userid='".$_SESSION['SES_LOGIN']."'", $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
				$useractSave=mysql_query("INSERT INTO user_act SET userid='".$_SESSION['SES_LOGIN']."', aktivitas='Menambah data transaksi pemasukan barang dengan nomor BPB: $txtBpb dan tanggal $cmbTglbpb', tanggal='".date("Y-m-d")."', jam='".date("G:i:s")."'") 
					 or die ("Gagal query".mysql_error());
				// Refresh form
				echo "<meta http-equiv='refresh' content='0; url=?page=Pemasukan-Barang&Act=Success'>";
...
$nomorTransaksi = buatKode("pemasukan", "000");
...
<form action="?page=Pemasukan-Barang" method="post"  name="frmadd">
<table width="100%" cellspacing="1" class="table-common" style="margin-top:0px;">
	<tr>
	  <td colspan="3" align="right"><h2>Incoming - Pemasukan barang dari pemasok</h2> </td>
	</tr>
	<tr>
    ...
    <tr><td colspan="3"><div align="left" class="style1">Header</div></td></tr>
      <tr>
	  <td width="30%" align="right"><b>Nomor dan tanggal Bukti Penerimaan Barang (BPB) </b></td>
	  <td width="1%"><b>:</b></td>
	  <td width="69%"><input name="txtBpb" id="txtBpb" readonly="readonly" value="<?php echo $nextNoTransaksi; ?>" size="45"   maxlength="100" onKeyPress="return tabE(this,event)"/> &nbsp;/&nbsp;<?php echo form_tanggal("cmbTglbpb",$tglBpb); ?></td></tr>
		<tr>
	  ...
	  <td><select name="cmbDokpab" onKeyPress="return tabE(this,event)">
        ...
        <?php
	  $dataSql = "SELECT kode,jenis FROM jenisbcmasuk ORDER BY kode";
	...
	<tr>
	  ...
	  ?>
    	...
    	<tr>
      <td align="right"><b>Catatan Pemasukan</b></td>
	  <td><b>:</b></td>
...
<tr>
	...
	<tr>
	  <td colspan="3"><b>Kode Barang </b><b>:</b><b>
					...
					<input type="text" name="txtKode" size="30" id="txtKode" onKeyPress="return tabE(this,event)" class="ui-widget ui-widget-content padding ui-corner-all button">
					<button id="kode-search" class="tombol">&nbsp;</button>
&nbsp;&nbsp;       
Qty :
<input class="angkaC" name="txtJumlah" onKeyPress="return tabE(this,event)" size="8" maxlength="12" value="1" 
...
&nbsp;&nbsp;
Nilai barang:
<input align="right" name="txtNilai" value="<?php echo $dataNilai; ?>" size="10" maxlength="30" onKeyPress="return tabE(this,event)" />
&nbsp;&nbsp;
Netto (kg):
<input align="right" name="txtNetto_detail" value="<?php echo $dataNetto_detail; ?>" size="10" maxlength="30" onKeyPress="return tabE(this,event)" />
&nbsp;&nbsp;
Volume (m3):
<input align="right" name="txtVolume_detail" value="<?php echo $dataVolume_detail; ?>" size="10" maxlength="30" onKeyPress="return tabE(this,event)" />
&nbsp;&nbsp;
   </td>
    </tr>
	<tr>
...
<?php
$tmpSql ="SELECT barang.kd_barang,barang.nm_barang,barang.satuan, tmp_pemasukan.*
		FROM tmp_pemasukan inner join barang on tmp_pemasukan.kode=barang.kd_barang
		WHERE tmp_pemasukan.userid='".$_SESSION['SES_LOGIN']."'
		ORDER BY tmp_pemasukan.id ";
$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Edit-Pemasukan-Barang&amp;ID=<?php echo $ID; ?>"  target="_self"><img src="images/b_edit.png" width="16" height="16" border="0" /></a></td>
    <td align="center" bgcolor="#FFFFCC"><a href="?page=Pemasukan-Barang&Act=Delete&ID=<?php echo $ID; ?>"  target="_self"><img src="images/hapus.gif" width="16" height="16" border="0" /></a></td>
  </tr>
<?php 
}?>
  <tr>
