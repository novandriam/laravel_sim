...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtNilai'])=="" OR ! is_numeric(trim($_POST['txtNilai']))) {
			$message[] = "Data <b>Harga Penyerahan barang (belum diisi</b>!";		
		}
		...
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE tmp_bc40 SET kode='$txtKode', jumlah='$txtJumlah', satuan='$cmbSatuan', harga_detail='$txtNilai', netto_detail='$txtNetto_detail', volume_detail='$txtVolume_detail' WHERE id='".$_POST['txtID']."'") 
					   or die ("Gagal query update".mysql_error());
			if($qryUpdate){
				echo "<meta http-equiv='refresh' content='0; url=?page=Tambah-BC40'>";
			}
			exit;
		}	
	...
	$sqlShow = "SELECT barang.kd_barang, barang.nm_barang, tmp_bc40.* FROM barang, tmp_bc40 WHERE tmp_bc40.id='$IDEdit' and tmp_bc40.kode=barang.kd_barang";
	...
	$dataNilai	= isset($dataShow['harga_detail']) ?  $dataShow['harga_detail'] : $_POST['txtNilai'];
...
?>
<form action="?page=Edit-BC40" method="post" name="frmedit">
<table class="table-list" width="100%" style="margin-top:0px;">
	<tr><input name="txtID" type="hidden" value="<?php echo $dataID; ?>" />
	  <td colspan="3"><h2>UBAH  DETAIL DATA BC 4.0</h2></td>
	</tr>
	...
	<tr>
	  ...
	  <td><select name="cmbSatuan">
          ...
          <?php
	  $dataSql = "SELECT kode,jenis FROM satuan ORDER BY kode";
