...
class mydb
{
	...
	function backupDatabase($name,$tables = '*')
	{
		...
		if (file_exists($fileName))
		{
			...
			$outputArchive = 'backup/backup/'.$zipName.'.zip'; 
			...
			$resultLink = "<a href='$outputArchive' target='_blank'>Klik here to DOWNLOAD DATABASE</a> or <a href='".$_SERVER['PHP_SELF']."?page=Backup-Database&cTask2=removeBckDB&file=$outputArchive'>remove backup database</a>.";
    	}
		echo $resultLink;
	}
	...
	function restoreDatabaseZip($sourceDB)
	{
		if(file_exists($sourceDB))
		{
			...
			if ($archive->extract(PCLZIP_OPT_PATH, 'backup/temp/') == 0)
			{
				die("Error : ".$archive->errorInfo(true));
			}
			$f = fopen('backup/temp/'.$this->getRealFileName($sourceDB).'.sql',"r+");
			$sqlFile = fread($f, filesize('backup/temp/'.$this->getRealFileName($sourceDB).'.sql'));
